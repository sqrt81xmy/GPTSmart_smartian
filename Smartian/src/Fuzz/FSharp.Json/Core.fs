...
module internal Core =
    ...
    let getJsonUnionCaseName (config: JsonConfig) (jsonUnion: JsonUnion) (jsonUnionCase: JsonUnionCase) (caseInfo: UnionCaseInfo) =
        // printfn "getJsonUnionCaseName %A %A" jsonUnionCase.Case caseInfo
    ...
    let rec deserialize (config: JsonConfig) (path: JsonPath) (t: Type) (jvalue: JsonValue): obj =
        ...
        let getUnionCaseInfo (path: JsonPath) (t: Type) (jCaseName: string): UnionCaseInfo =
            ...
            // printfn "getUnionCaseInfo-unionCases: %A %s" unionCases jCaseName
            // let mmkk = getJsonUnionCaseName config jsonUnion (getJsonUnionCase unionCases[1]) unionCases[1]
            // printfn "jsonUnicases: %s %s" mmkk jCaseName
            let caseInfo = unionCases |> Array.tryFind (fun c -> getJsonUnionCaseName config jsonUnion (getJsonUnionCase c) c = jCaseName)
            // printfn " caseInfo %A" caseInfo
            match caseInfo with
        ...
        let deserializeUnion (path: JsonPath) (t: Type) (jvalue: JsonValue): obj =
            ...
            | _ ->
                ...
                //     FSharpValue.MakeUnion (caseName |> getUnionCaseInfo path t, [| box 32 |])
                
            match jvalue with
                | JsonValue.String caseName ->
                    let caseInfo = getUnionCaseInfo path t caseName 
                    match caseName with
                    | "Int" -> 
                        FSharpValue.MakeUnion(caseInfo, [| box 32 |])  // 传递与 Int 相关的 ByteWidth
                    | "UInt" -> 
                        FSharpValue.MakeUnion(caseInfo, [| box 32 |])  // 传递与 UInt 相关的 ByteWidth  
                    | "Untouched" -> 
                        FSharpValue.MakeUnion(caseInfo, [| box 0uy |])  // 传递与 UInt 相关的 ByteWidth  
                    | _ -> 
